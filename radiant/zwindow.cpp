...
void ZWnd::OnLButtonDown( guint32 nFlags, int pointx, int pointy ){
	SetFocus();
	SetCapture();
	Z_MouseDown( pointx, gtkutil_widget_get_height( m_pWidget ) - 1 - pointy, nFlags );
}
...
void ZWnd::OnLButtonUp( guint32 nFlags, int pointx, int pointy ){
	Z_MouseUp( pointx, gtkutil_widget_get_height( m_pWidget ) - 1 - pointy, nFlags );
	ReleaseCapture();
}
...
void ZWnd::OnMouseMove( guint32 nFlags, int pointx, int pointy ){
	float fz = z.origin[2] + ( ( gtkutil_widget_get_height( m_pWidget ) - 1 - pointy ) - ( z.height / 2 ) ) / z.scale;
	...
	Z_MouseMoved( pointx, gtkutil_widget_get_height( m_pWidget ) - 1 - pointy, nFlags );
}
